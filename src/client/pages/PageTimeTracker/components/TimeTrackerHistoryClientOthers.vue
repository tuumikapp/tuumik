...
async function refreshList(limit2) {
  if (props.time.clientId) {
    limit.value = limit2;
    loadingList.value = true;
    const clientId = props.time.clientId;
    try {
      const res = await Meteor.callAsync('timeHistoryClientOthers', clientId, limit2);
