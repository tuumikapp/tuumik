...
async function refreshList(limit2) {
  if (props.time.projectId) {
    limit.value = limit2;
    loadingList.value = true;
    const timeId = props.time._id;
    const projectId = props.time.projectId;
    try {
      const res = await Meteor.callAsync('timeHistoryProjectSelf', timeId, projectId, limit2);
